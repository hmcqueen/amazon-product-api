...
const REVIEW_ASIN_REGEX = /product-reviews\/(\w+)\//;
function parseReview(asin, id, el, dateParser) {
    const result = {
        id,
        asin: {
            original: asin,
     ...
     */
    try {
        const reviewDateString = el.find('[data-hook="review-date"]')[0].children[0].data;
        result.date = dateParser(reviewDateString);
    } catch (error) { }
     ...
     */
    try {
        const reviewAsinUrl = el.find('div.a-row.a-spacing-mini.review-data.review-format-strip > a')[0].attribs.href
        const match = REVIEW_ASIN_REGEX.exec(reviewAsinUrl);
        if (match) {
            result.asin.variant = match[1];
        }
    } catch (error) { }
     ...
     */
    try {
        result.name = el.find('.a-profile-name')[0].children[0].data;
    } catch (error) { }
     ...
     */
    try {
        result.rating = parseFloat(el.find('[data-hook="review-star-rating"]')[0].children[0].children[0].data.split(' ')[0]);
    } catch (error) { }
     ...
     */
    try {
        result.title = el.find('[data-hook="review-title"]').text();
    } catch (error) { }
     ...
     */
    try {
        result.review = el.find('[data-hook="review-body"]').text();
    } catch (error) { }
     ...
     */
    try {
        result.verified_purchase = !!el.find('[data-hook="avp-badge"]').length;
    } catch (error) { }
     ...
     */
    try {
        const mediaEl = el.find('.review-image-tile-section')[0];
        result.media = mediaEl
            ? mediaEl.children.map((item) => {
...
class AmazonScraper {
    ...
    get setRequestEndpoint() {
        switch (this.scrapeType) {
            case 'products':
                return 's';
            case 'reviews':
                return `product-reviews/${this.asin}/ref=cm_cr_arp_d_viewopt_srt?ie=UTF8&formatType=${
    ...
    async buildRequest(page) {
        const url = this.setRequestEndpoint;
    ...
    grabReviews(body) {
        ...
        for (let i = 0; i < reviewsList.length; i++) {
             ...
             * Process reviews
             */
            scrapingResult[reviewId] = parseReview(this.asin, reviewId, $(`#${reviewId}`), this.geo.review_date)
        }
